 ...
 */
public class FPCFP60KL  extends FPCBase {

    private CS_BGR_FP60_KL FP;
    private int lastErrorCode;
    private String lastCommand;
    

    public FPCFP60KL() throws Exception {
        super();
    }

    public static String getClassDecription() {
        return "This class supports Datecs FP-60 KL fiscal printer!";
    }
    ...
    public static FPParams getDefinedParams() throws Exception {
        ...
        params.addGroups(
                new FPPropertyGroup("main", "Main Options") {{
                     ...
                     addProperty(new FPProperty(Integer.class, "LWIDTH", "Line width", "Line width in characters", 36));
                }}
    ...
    protected String taxGroupToChar(FPCBase.taxGroup taxCode) {
        String res; 
        switch (taxCode) {
            case A : res = "А"; break;
            case B : res = "Б"; break;
    ...
    protected String taxGroupToCharL(FPCBase.taxGroup taxCode) {
        String res; 
        switch (taxCode) {
            case A : res = "A"; break;
            case B : res = "B"; break;
    ...
    protected String paymenTypeChar(FPCBase.paymentTypes payType) {
...
*/        
    ...
    public void abnormalComplete() throws FPException {
        ...
        LinkedHashMap<FPCBase.taxGroup,String[]> tax = new LinkedHashMap<>();
        tax.put(FPCBase.taxGroup.A, new String[]{""});
        tax.put(FPCBase.taxGroup.B, new String[]{""});
        tax.put(FPCBase.taxGroup.C, new String[]{""});
        tax.put(FPCBase.taxGroup.D, new String[]{""});
        tax.put(FPCBase.taxGroup.E, new String[]{""});
        tax.put(FPCBase.taxGroup.F, new String[]{""});
        tax.put(FPCBase.taxGroup.G, new String[]{""});
        tax.put(FPCBase.taxGroup.H, new String[]{""});
...
//	public int cMD_103_0_0(String[] canVd, String[] taxA, String[] taxB, String[] taxC, String[] taxD, String[] taxE, String[] taxF, String[] taxG, String[] taxH, String[] inv, String[] invNmb) 
        lastErrorCode = FP.cMD_103_0_0(
            canVoid
            , tax.get(FPCBase.taxGroup.A), tax.get(FPCBase.taxGroup.B), tax.get(FPCBase.taxGroup.C), tax.get(FPCBase.taxGroup.D)
            , tax.get(FPCBase.taxGroup.E), tax.get(FPCBase.taxGroup.F), tax.get(FPCBase.taxGroup.G), tax.get(FPCBase.taxGroup.H)
            , isInv, invDocNum
        );
        if (lastErrorCode == 0) {
            if (canVoid[0].length() != 0 && !canVoid[0].equals("0")) {
                ...
                double tSum; 
                for(FPCBase.taxGroup tg : tax.keySet()) {
               ...
               // try to pay rests
                ...
                if (tSum > 0) {
                    ...
                    lastErrorCode = FP.cMD_53_0_0("Общо", "", paymenTypeChar(FPCBase.paymentTypes.CASH), Double.toString(tSum), paidCode, amountOut);
                }    
                // close fiscal check
    ...
    @Override
    public void sell(String text, FPCBase.taxGroup taxCode, double price, double quantity, double discountPerc)  throws FPException {
    ...
    @Override
    public void sell(String text, FPCBase.taxGroup taxCode, double price, double discountPerc) throws FPException {
    ...
    public StrTable subTotal(boolean toPrint, boolean toDisplay, double discountPerc) throws FPException {
        ...
        for(FPCBase.taxGroup taxCode : tax.keySet())
            res.put("Tax"+taxGroupToCharL(taxCode), tax.get(taxCode)[0]);
        return res;
    }

    @Override
    public StrTable total(String text, FPCBase.paymentTypes payType, double payAmount) throws FPException {
        String[] lines = splitOnLines(text);
        String[] paidCode = new String[]{""};
        String[] amountOut = new String[]{""};
...
//        res.put("LFRI_Total", Double.toString(total[0]));
    ...
    @Override
    public StrTable reportDaily(FPCBase.dailyReportType reportType) throws FPException {
	...
	// public int cMD_69_0_0(
        ...
        lastErrorCode = FP.cMD_69_0_0(
            (reportType == FPCBase.dailyReportType.Z)?"0":"2", ""
            , closure, fM_Total
            , tax.get(FPCBase.taxGroup.A), tax.get(FPCBase.taxGroup.B), tax.get(FPCBase.taxGroup.C), tax.get(FPCBase.taxGroup.D)
            , tax.get(FPCBase.taxGroup.E), tax.get(FPCBase.taxGroup.F), tax.get(FPCBase.taxGroup.G), tax.get(FPCBase.taxGroup.H)
        );
    ...
    @Override
    public StrTable reportByDates(Date from, Date to, FPCBase.datesReportType reportType) throws FPException {
        StrTable res = new StrTable();
        SimpleDateFormat dateFormat = new SimpleDateFormat("ddMMyy");
        if (reportType == FPCBase.datesReportType.DETAIL) {
